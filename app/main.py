...
# 导入路由
from app.routes import auth, admin_team as admin, api
...
app = FastAPI(
    title="GPT Team 管理系统",
    description="ChatGPT Team 账号与成员管理系统",
    version="0.1.0",
    lifespan=lifespan
)
...
async def root_redirect(request: Request):
    """Team 管理版首页：未登录进入登录页，已登录进入 Team 控制台。"""
    target = "/admin" if request.session.get("user", {}).get("is_admin") else "/login"
